...
## 🚀 A Solução: "Prompt de Planejamento do Gemini Agent Ecosystem"
...
### ❌ Antes (Instrução Básica de Planejamento)

```text
PM: "Estamos pensando em colocar os agentes do Gemini no nosso app de reservas de viagens. Me dê algumas ideias."

IA (Resposta Genérica e Ultrapassada):
1. Adicionar Chatbot: Os usuários podem conversar com o Gemini dentro do app para receber recomendações de viagens.
2. Autocompletar texto: O app pode sugerir frases quando o usuário estiver escrevendo uma avaliação de hotel.
... (A IA apenas lista recursos antigos de LLM, ignorando totalmente o paradigma da integração de agentes em nível de Sistema Operacional).
```

### ✅ Depois (Resultado aplicando o Prompt Pro)

```text
**1. Caso de Uso Core: Correspondência Automática de Menor Preço em Segundo Plano e Checkout em Espera**
- **Cenário de UX:**
  1. Comando de Voz do Usuário: "Reserve um hotel executivo na faixa de R$ 300 em São Paulo para minha viagem de negócios de amanhã." (Emitido a partir da Tela Inicial do OS).
  2. Ação do Agente: O Agente verifica a agenda no calendário do usuário → Aciona o nosso aplicativo (via Intent) para buscar opções em segundo plano → Cria um Deep Link direto para a página final de pagamento.
  3. Ação do Usuário: O usuário visualiza o resumo da reserva gerado pela IA na tela e confirma: "Pode pagar" (Autenticação biométrica é acionada e concluída).

**2. Requisitos de Integração do Gemini Agent SDK**
- **Definição de Intents:** Expor de forma programática as intents customizadas `ACTION_SEARCH_HOTEL` e `ACTION_PREPARE_CHECKOUT` no AndroidManifest.
- **UI Understanding (Compreensão da Interface):** Aplicar a tag `<semantic-tag role="price">` aos componentes de preço e às listagens de avaliações, a fim de garantir que o Agente de Visão da IA os identifique sem margem de erro.
- **Segurança de Dados (Nano v3):** Exigir que todos os tokens de pagamento e dados detalhados da agenda do usuário utilizem exclusivamente processamento on-device e comunicação criptografada, sem trânsito via nuvem externa.
```
