...
# (Updated) 📝 이 코드, 무슨 뜻이죠? 친절한 코드 해설사 {#updated}
...
## (Updated) 🚀 해결책: "코드 난독화 해제기" {#updated}
...
### (Updated) ❌ Before (입력 - Python) {#python}
...
```python
# (Updated) 0부터 9까지(range(10)) 숫자를 하나씩 꺼내 변수 x에 담습니다. {#range-10}
# (Updated) 만약(if) x를 2로 나눈 나머지가 0이라면(짝수라면), {#if}
# (Updated) x에 2를 곱한 값을 리스트에 담습니다. {#updated}
result = [x * 2 for x in range(10) if x % 2 == 0]
```
