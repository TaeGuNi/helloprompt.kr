...
category: "데이터/분석"
description: "Trabalhando até tarde organizando planilhas? Descubra o prompt perfeito para a IA limpar e formatar seus dados em segundos, sem usar fórmulas complexas."
...
Cerca de 80% do tempo gasto em análise de dados e rotinas administrativas é consumido pelo **pré-processamento (Pre-processing)**. Endereços inconsistentes, números de telefone com ou sem hífen e nomes de clientes cheios de erros de digitação: você ainda corrige tudo isso manualmente ou criando fórmulas intermináveis no Excel?

A partir de agora, basta fornecer à IA **padrões e regras** claros. Ela identifica exceções que costumamos ignorar e organiza tudo automaticamente. Apresentamos o prompt definitivo para libertá-lo de tarefas repetitivas, permitindo que você tenha foco total no seu verdadeiro trabalho estratégico.
...
A grande sacada aqui é processar dezenas ou centenas de linhas diretamente na janela do chat. No entanto, para grandes volumes de dados (milhares de linhas), o ideal é solicitar um **código em Python para automatizar a conversão**.

> **Papel (Role):** Você atua como um `[Engenheiro de Dados]` Sênior com 10 anos de experiência.
>
> **Contexto (Context):**
>
> - Cenário: Tenho um arquivo CSV de clientes com dados em formatos completamente inconsistentes.
> - Objetivo: Pré-processar esses dados para um formato limpo, pronto para ser usado imediatamente em análises e marketing.
>
> **Tarefa (Task):**
>
> 1. Analise os dados de entrada fornecidos e padronize-os perfeitamente seguindo as "Regras de Limpeza" abaixo.
> 2. Pensando no processamento de grandes volumes no futuro, escreva um script de automação em `[Python Pandas / Excel VBA]` aplicando essas mesmas regras.
>
...
> - Não invente informações para dados que não podem ser inferidos. Mantenha o valor original ou marque como "Verificar" (Prevenção contra alucinação).
>
...
- **P: O resultado final voltou com menos linhas que o arquivo original. Por que isso acontece?**
  - R: Isso ocorre porque a IA pode ter julgado arbitrariamente que linhas "defeituosas" ou muito fora do padrão eram irrelevantes e as deletou. Para evitar isso, a frase contida nas *Restrições*: **"O número de linhas dos dados originais e o do resultado final devem ser exatamente iguais"** é de extrema importância.
...
Limpar dados complexos não precisa mais ser um trabalho exaustivo para os seus olhos e mãos. Delegue essa tarefa maçante à IA, a verdadeira especialista no reconhecimento de padrões e aplicação rigorosa de regras.
