...
category: Design
description: "Überlassen Sie komplexe Bezier-Kurven der KI! Generieren Sie natürliche, butterweiche CSS-Animationen in Sekundenschnelle und sparen Sie wertvolle Zeit."
...
> _"Der Button soll beim Klicken sanft größer werden, wie Wackelpudding federn und in Regenbogenfarben leuchten." – Graut es Ihnen bei solchen abstrakten Design-Anfragen auch vor stundenlangem Keyframe-Basteln?_

CSS-Animationen wirken oft roboterhaft und billig, wenn die Werte auch nur minimal danebenliegen. Um natürliche, "physikalische" Bewegungen und butterweiche Übergänge in Code zu gießen, bedurfte es bisher endloser Reloads und frustrierender Trial-and-Error-Schleifen. Befreien Sie sich von der Qual, in den DevTools mühsam Cubic-Bezier-Kurven hinzubiegen! Beschreiben Sie einfach in menschlicher Sprache, welches Gefühl Sie erzeugen möchten – und die KI liefert Ihnen im Handumdrehen butterweichen, auf 60 FPS optimierten CSS-Code.

---

## ⚡️ 3-Punkte-Zusammenfassung (TL;DR)

1. Generieren Sie komplexe CSS-Keyframe-Animationen in Sekundenschnelle durch einfache Sprachbeschreibungen.
2. Erhalten Sie perfekt abgestimmte `cubic-bezier`-Werte für geschmeidige, organisch wirkende Mikro-Interaktionen.
3. Verhindern Sie Performance-Einbrüche durch strikt GPU-beschleunigten, Reflow-vermeidenden Code.
...
Nutzen Sie diesen Prompt, wenn Sie schnell funktionierenden Code für einfache Bewegungsabläufe benötigen.
...
Verwenden Sie diesen Prompt für detaillierten, produktionsreifen Code, der natürliche Bewegungsdynamik mit kompromissloser Hardwarebeschleunigung vereint.
...
> - Ziel: `[Erstellung von absolut flüssigen, ruckelfreien (jank-free) 60-FPS-CSS-Animationen in höchster Qualität]`
>
> **Aufgabe (Task):**
>
> 1. Schreibe detaillierten, feingetunten `@keyframes`-Code basierend auf der untenstehenden **[Animationsbeschreibung]**.
> 2. Liefere den CSS-Code in Form einer `.class`, damit die Animation sofort auf ein Element angewendet werden kann.
> 3. Anstelle eines langweiligen `ease-in-out` empfehle und verwende einen raffinierten, benutzerdefinierten `cubic-bezier`-Wert, der das gewünschte 'Gefühl' der Bewegung perfekt einfängt.
>
...
- **F: Kann ich diesen Prompt auch in Tailwind CSS- oder Styled-Components-Umgebungen nutzen?**
  - A: Absolut! Fügen Sie einfach unter *Kontext* oder *Aufgabe* einen Satz hinzu wie: "Schreibe den Code unter Verwendung der Tailwind CSS `animate-` Utility-Klassen und der passenden `tailwind.config.js` Konfiguration" oder "Verwende die `keyframes`-Hilfsfunktion von Styled-Components". Die KI liefert Ihnen daraufhin perfekt kompatiblen Code.

- **F: Lässt sich das auch in Framer Motion (React) oder Vue Transition umschreiben?**
  - A: Ja, problemlos. Wenn Sie eine spezifische Bibliothek bevorzugen – weisen Sie die KI einfach an: "Schreibe den Code als React Framer Motion-Komponente unter Verwendung von `initial`, `animate` und `transition`". Die KI nutzt die Stärken dieser Syntax und liefert optimierten Komponenten-Code.

- **F: Ich habe den KI-Code eingefügt, aber die Animation ruckelt. Was kann ich tun?**
  - A: Zu 99 % liegt dieser Performance-Einbruch daran, dass layoutverändernde Eigenschaften (Reflow) wie `width`, `height`, `top` oder `left` direkt in den Keyframes animiert wurden. Stellen Sie sicher, dass Sie die KI – wie in der Pro-Version beschrieben – strikt dazu zwingen, **ausschließlich** `transform: translate()` und `opacity` zu verwenden.

---

## 🧬 Anatomie des Prompts (Warum er funktioniert)

1. **Experten-Persona (Role):** Die KI wird nicht als bloßer Code-Generator, sondern als "Senior Interaction Designer" positioniert. Das führt zu ästhetisch deutlich anspruchsvolleren und detaillierteren Ergebnissen.
...
3. **Performance-Fokus (Constraints):** Um das größte Problem im Frontend-Alltag – Animationsruckeln (Jank) – von vornherein auszuschließen, gibt es eine harte Vorgabe, ausnahmslos GPU-beschleunigte Eigenschaften zu nutzen.
...
Die Zeiten, in denen man beim Anblick eines Designentwurfs verzweifelt dachte: "Welche Bezier-Kurve brauche ich nur für diese federnde Bewegung?", sind endgültig vorbei.
Beschreiben Sie Ihr Gespür für Bewegung einfach in natürlicher Sprache und überlassen Sie die komplexen physikalischen Berechnungen sowie die Performance-Optimierung der KI.
