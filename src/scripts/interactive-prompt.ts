...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (bookmarkBtn) {
    ...
    const isBookmarked = bookmarks.some(
      (b: Record<string, unknown>) => b.path === currentPath,
    );
    if (isBookmarked) {
  ...
  wrappers.forEach((wrapper) => {
    ...
    if (wrapper.classList.contains("quote-wrapper")) {
      ...
      if (blockquote) {
        ...
        if (placeholderRegex.test(html)) {
          html = html.replace(placeholderRegex, (match, _full, innerText) => {
    ...
    const getFinalText = () => {
      ...
      const originalInputs = element.querySelectorAll("input.prompt-input");

      // Replace inputs with their values or placeholders in the clone
      inputs.forEach((input, index) => {
        const userValue = (
          originalInputs[index] as HTMLInputElement
        ).value.trim();
    ...
    if (wrapper.classList.contains("quote-wrapper")) {
      ...
      actionContainer.appendChild(
        createActionBtn("Grok", "https://x.com/i/grok", "🚀 Grok", "btn-grok"),
      );
