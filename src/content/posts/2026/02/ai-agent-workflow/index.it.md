...
```mermaid
graph TD;
    A[Avvio Schedulato 08:00 AM] --> B{Web Search API: Ricerca News};
    B -->|Successo| C[Analisi Sentiment e Punti Chiave];
    B -->|Errore/Timeout| D[Attiva Web Scraping Python Alternativo];
    D --> C;
    C --> E[Generazione Report Esecutivo in Markdown];
    E --> F{Email API: Invio al Management};
    F -->|Successo| G[Registrazione Log Positivo];
    F -->|Errore| H[Invio Alert di Emergenza su Slack IT];
```
