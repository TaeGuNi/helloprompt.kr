...
# 📝 O Wi-Fi Pode se Tornar uma Câmera de Vigilância Invisível?
...
## 🚀 A Solução: "Consultor de Privacidade Wi-Fi Sensing"

### 🥉 Versão Básica (Basic Version)

Ideal para entender o conceito rapidamente e aplicar configurações simples em roteadores domésticos.
...
Para uma análise de ameaças profunda, focada na auditoria de rede corporativa ou configurações avançadas de RF (Radiofrequência).

> **Função (Role):** Você é um `[Arquiteto de Segurança Cibernética e Especialista em Radiofrequência]`, focado em tecnologias de vigilância emergentes.
>
> **Contexto (Context):**
>
> - Cenário: `[Estou configurando a infraestrutura de rede (Mesh/Access Points) da minha casa/escritório e quero mitigar os riscos de vigilância passiva através de sinais RF/Wi-Fi]`.
> - Objetivo: `[Receber um relatório técnico de avaliação de risco e um plano de mitigação focado em ofuscação de sinal e controle de transmissão]`.
>
> **Tarefa (Task):**
>
> 1. Explique como os metadados da camada física (como o CSI - Channel State Information) são processados por IA para rastreamento de presença.
> 2. Liste os potenciais vetores de ataque se um invasor interceptar o sinal fora da minha propriedade.
> 3. Crie um guia passo a passo prático para implementar medidas defensivas com base nas configurações padrão do roteador, como o `[Controle de Potência de Transmissão (Tx Power)]` e topologia física.
>
...
> **Aviso (Warning):**
>
> - Baseie-se exclusivamente em artigos científicos validados e padrões técnicos (ex: IEEE 802.11bf). Se não houver uma defesa técnica conhecida para um ataque específico, seja honesto e afirme "atualmente sem mitigação comprovada via software".
...
### ❌ Antes (Busca Genérica no Google)

```text
Como o Wi-Fi me espiona e como me proteger?
(Resultados focados em provedores de internet que vendem histórico de navegação, sugerindo a compra de antivírus ou VPN, ignorando o rastreamento físico do ambiente).
```

### ✅ Depois (Resultado do Prompt Pro)

```text
### Relatório de Mitigação: Rastreamento por Radiofrequência e CSI

**1. A Mecânica do Rastreamento (Wi-Fi Sensing)**
O roteador emite sinais continuamente para detectar dispositivos. Quando essas ondas atingem um ser humano (uma massa de água ambulante), a amplitude e a fase das ondas sofrem alterações micrométricas (Channel State Information). Algoritmos de Machine Learning decodificam esses ecos, mapeando movimentações complexas sem qualquer dispositivo óptico.

**2. Plano de Ação Prático**
- **Controle de Tx Power (Potência de Transmissão):** Acesse a interface web do seu roteador (192.168.1.1). Localize "Wireless Advanced Settings" e reduza o 'Transmit Power' de 100% para 50% ou 75%. O objetivo é que o sinal cubra a casa, mas não chegue à rua.
- **Isolamento Geométrico:** Posicione os roteadores de forma centralizada ou apontados do exterior para o centro da casa, minimizando o vazamento de radiofrequência para fora dos seus muros físicos.
```
