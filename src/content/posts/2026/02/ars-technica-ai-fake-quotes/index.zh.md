...
description: "Ars Technica 在发现文章中包含伪造的 Matplotlib 维护者引言后撤回了报道。这一事件凸显了新闻业盲目滥用 AI 的巨大风险。本文将为您提供一套实用的事实核查提示词，彻底杜绝 AI 幻觉。"
lang: "zh"
---
...
著名的科技新闻网站 **Ars Technica** 近期在发现其文章中包含完全伪造的开源项目 Matplotlib 维护者引言后，紧急撤回了相关报道。这起事件虽然由经验丰富的资深作者（Benj Edwards 与 Kyle Orland）撰写，却被社区敏锐的读者（Hacker News 与 Mastodon 用户）迅速揭露。维护者本人更是亲自下场辟谣：“我从未那样说过。”
...
2. **声誉损失惨重**：开源社区用户迅速揭露了真相，这反映出在追求出稿速度的过程中，传统事实核查机制正在受到严重破坏。
...
> - 目标：确保草稿中的每一句引言、每一个技术细节和数据都具备绝对的真实性，彻底杜绝任何捏造的言论或无中生有的事实。
>
> **任务 (Task)：**
>
...
> 3. **核查建议**：针对每一个疑点，提供给作者具体的交叉验证建议（例如：“请查阅开发者在 GitHub 或 Mastodon 上的原始发帖”）。
>
> **约束条件 (Constraints)：**
>
...
这套 Pro 版提示词的核心价值在于**“角色反转（Role Reversal）”**——我们不再让 AI 去**写**新闻，而是让大模型扮演一个极其苛刻的同行去**挑刺**。通过在发布前强制增加这一道“红蓝对抗”工序，不仅能大幅度筛查出潜在的公关危机，还能倒逼创作者在查阅原始资料时始终保持敬畏之心。
...
- **Q: 为什么推荐使用逻辑推理能力强的模型（如 Claude 3.5 Sonnet）？**
  - A: 在事实核查场景中，模型需要理解极其微妙的语境突兀感。较弱的模型往往看不出同类模型生成的幻觉，而顶级模型在文本逻辑分析和批判性阅读方面表现更为卓越，能敏锐察觉出“过度圆滑的公关语言”。
