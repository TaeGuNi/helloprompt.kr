...
> **Contexte (Context) :**
> 
> - **Projet :** `[Application de scan de déchets alimentaires et de gestion des stocks d'ingrédients basée sur l'IA]`
...
### ❌ Avant (Style "Journal intime")

```text
Quand j'ai commencé à vivre seul, j'ai trouvé que jeter les restes de nourriture était vraiment embêtant et que ça sentait mauvais. Donc, je veux créer une application où l'on prend en photo les ingrédients dans le frigo, ça nous donne la date de péremption, et ça propose des recettes avec ce qu'il reste.
```

### ✅ Après (Style Business Plan gagnant)

```text
[1-1. Identification du problème (Problem)]
- Augmentation du besoin d'ingrédients en petites portions suite à la hausse du nombre de foyers d'une personne, et coût annuel de traitement des déchets alimentaires extrêmement élevé.
- Limites de la gestion manuelle des stocks entraînant un taux de gaspillage alimentaire approchant les 30 % chez les personnes seules, générant des pertes économiques continues.

[1-2. Solution (Solution)]
- Développement d'une solution de "Scan du réfrigérateur en une prise et création automatique d'une base de données des dates de péremption" utilisant la technologie de reconnaissance d'objets AI Vision.
- Réalisation d'un "Food Upcycling" grâce à un algorithme de recommandation de recettes personnalisées basées sur les ingrédients dont la date limite de consommation est imminente.
```
