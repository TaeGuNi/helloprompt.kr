...
>
> - Le diagramme ASCII doit être suffisamment étroit pour s'afficher correctement sur un écran de smartphone sans provoquer de défilement horizontal.
> - L'analogie choisie doit être la plus précise et pertinente possible par rapport au concept d'origine.
>
> **Attention (Warning) :**
>
> - Si certains aspects du concept sont impossibles à schématiser simplement, mentionne-le de manière transparente plutôt que de générer un diagramme trompeur (pas d'hallucinations).
...
- **Q : Les diagrammes ASCII s'affichent mal sur mon téléphone, que faire ?**
...
```text
Explique-moi le Handshake TCP/IP (le protocole de connexion à 3 voies).
```
...
[Diagramme ASCII]
  ...
  [Client]                [Serveur]
     |                       |
     |---- (1) SYN --------->|  "Bonjour ! Es-tu disponible ?"
     |                       |
     |<--- (2) SYN + ACK ----|  "Oui, je suis là et prêt !"
     |                       |
     |---- (3) ACK --------->|  "Parfait, commençons l'échange !"
     |                       |
     |==== [ Connexion ] ====|
...
C'est exactement comme passer un coup de téléphone important :
1. SYN : Vous composez le numéro et ça sonne. (Demande de connexion)
2. SYN + ACK : La personne décroche et dit "Allô ?". (Confirmation de réception et de disponibilité)
3. ACK : Vous répondez "Bonjour, c'est moi !". (Confirmation finale, la vraie conversation peut commencer)
```
