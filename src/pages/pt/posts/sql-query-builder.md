...
데이터 추출이 필요한데 SQL 문법이 기억나지 않아 고생한 적 있나요? 이 프롬프트는 자연어로 요청하면 정확한 SQL 쿼리를 생성해주고, 쿼리의 작동 R$리까지 설명해줍니다.
...
> **요청 (Task):**
>
> 1. `[R$하는 데이터]`를 추출하는 효율적인 SQL 쿼리를 작성해줘.
...
```text
users 테이블이랑 orders 테이블 합쳐서, 2024년에 가입한 유저 중에 주문 금액이 10만R$ 이상인 사람 이름이랑 총 주문금액 보여줘.
```
