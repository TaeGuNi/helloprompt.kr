...
Los *bugs* más aterradores son aquellos que fallan en silencio o que escupen mensajes de error tan crípticos que parecen un callejón sin salida. ¿Llevas horas sufriendo en solitario porque el problema es tan enredado que ni siquiera sabes cómo explicárselo a un colega? Si le entregas los *logs* y el código a la IA adecuada, esta actuará como un auténtico detective y descubrirá la causa raíz en el rincón que menos esperas.
...
> **Solicitud:** Analiza este código `[Pega aquí el código]` junto con el siguiente error `[Pega aquí el error]`. Identifica la causa raíz y proporcióname la solución exacta.
...
> **Situación (Context):**
>
> - Contexto: Ha ocurrido un error inesperado y crítico que bloquea mi aplicación.
...
Este *prompt* es un auténtico salvavidas, especialmente cuando te toca lidiar con código *legacy* heredado o cuando trabajas en arquitecturas de microservicios donde los errores tienden a evaporarse silenciosamente entre distintas capas.

Lo verdaderamente brillante de exigir **"3 hipótesis"** es que evitas que la IA se obsesione con la primera conclusión evidente (que, con demasiada frecuencia, resulta ser errónea). Al forzarla a contemplar múltiples vectores de fallo, aumentas exponencialmente la probabilidad de destapar problemas de concurrencia, *race conditions* o estados asíncronos mal gestionados; detalles sutiles que un ojo humano, fatigado tras horas de frustrante depuración, pasaría por alto sin darse cuenta.
...
3. **Warning (Seguridad):** Introduce una capa de protección indispensable al exigir que la IA censure y ofusque cualquier dato sensible que haya podido filtrarse accidentalmente en los *logs*.
