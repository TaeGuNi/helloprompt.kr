...
category: "프롬프트 엔지니어링"
description: "Cansado de IAs entregarem um 'Sucesso!' sem alma? Um cheat code hardcore que força cenários de destruição para testar a resiliência do seu sistema."
...
- **⏱️ Tempo Necessário:** De 10 minutos → para 10 segundos (Corta a enrolação da IA pela raiz)
...
Peça a um agente de IA para escrever testes e, em 9 de 10 vezes, ele entregará apenas cenários de 'caminho feliz' perfeitamente idílicos. É um conto de fadas onde APIs externas respondem em 0.1 milissegundos, o banco de dados nunca pisca e o usuário sempre insere os dados imaculados. Esqueça isso. O código que realmente precisamos é um escudo intransponível, capaz de repelir 80% das falhas críticas e desastres em produção. O *cheat code* que apresentamos aqui é um prompt hardcore: ele agarra a IA pelo colarinho e injeta, à força, a implacável filosofia do 'Chaos Monkey' da Netflix na sua suíte de testes.
...
> 2. Você DEVE incluir, no mínimo, um dos seguintes cenários de desastre: *Timeout* de comunicação com API externa, HTTP 500 Internal Server Error ou Desconexão abrupta do Banco de Dados.
...
> - NUNCA polua objetos globais (*Dirty Mocking*)! Se você tentar sobrescrever `global.fetch` ou `window.setTimeout` de forma direta, eu matarei o processo imediatamente.
...
Sejamos francos. Quando você pede a uma IA para escrever testes, ela foge para uma utopia particular só para exibir aquele 'check verde (✅)' na tela. Um mundo mágico onde APIs respondem em 1ms e usuários só enviam payloads JSON imaculados. 

Este prompt incendeia essa utopia. No mundo real, servidores não caem por erros de lógica bobos; eles colapsam porque aquela API de terceiros "super confiável" de repente cospe um 502 Bad Gateway, ou porque o sistema entra num *loop* de carregamento infinito e estúpido quando as conexões do AWS RDS oscilam. Ao usar este *cheat code* do Chaos Monkey, a IA será forçada a escavar ativamente os *Edge Cases* mais obscuros e a bloquear totalmente táticas de *mocking* amadoras que poluem o escopo global. É, literalmente, a única vacina que vai salvar seus finais de semana e madrugadas de *debugging* desesperado.
...
- **Resultado:** 
  - Valida rigorosamente se um `Timeout Error` é disparado corretamente quando a rede atrasa mais de 5 segundos.
  - Verifica se uma *Fallback UI* de segurança é renderizada para o usuário quando o banco de dados cai e retorna HTTP 500.
...
Código de teste não é escrito para lhe dar paz de espírito. Ele é uma simulação brutal, projetada para provar se a sua aplicação consegue sobreviver no inferno absoluto que é o ambiente de produção real. 
