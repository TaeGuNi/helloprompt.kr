...
export function generateTrustReport(
  ...
  const receipt: VirtualReceipt = {
    base_compute:
      pricingResult.components.find((c) => c.key === "base_compute")?.cost || 0,
    reasoning_cost:
      pricingResult.components.find((c) => c.key === "reasoning_cost")?.cost ||
      0,
    verification_cost:
      pricingResult.components.find((c) => c.key === "verification_cost")
        ?.cost || 0,
    formatting_cost:
      pricingResult.components.find((c) => c.key === "formatting_cost")?.cost ||
      0,
