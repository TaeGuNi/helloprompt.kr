---
layout: /src/layouts/Layout.astro
title: "Aggiungere Magnificenza 'Angolo Basso' (Vista a Occhio di Verme)"
author: "Zzabbis"
date: "2026-02-04"
updatedDate: "2026-02-04"
category: "Generazione Immagini"
description: "Prompt angolo basso che enfatizza dignità travolgente e dimensione guardando il soggetto dal basso."
tags:
  [
    "Midjourney",
    "DALL-E",
    "Stable Diffusion",
    "Angolo Telecamera",
    "Composizione",
  ]
---

# 📝 Sguardo di un Gigante: Angolo Basso

**🎯 Consigliato per:** Tutti
**⏱️ Tempo richiesto:** 5 minuti

- **🤖 Modello consigliato:** Tutti i modelli IA

| Difficoltà | Efficacia  |  Utilità  |
| :--------: | :--------: | :-------: |
|  ⭐⭐☆☆☆   | ⭐⭐⭐⭐⭐ | ⭐⭐⭐⭐☆ |

_"Voglio far sembrare il personaggio più forte e più grande."_

**Angolo Basso** (o **Vista a Occhio di Verme**), attaccando la fotocamera a terra e guardando in alto, fa sembrare il soggetto più enorme, più minaccioso e autorevole del reale. Essenziale quando si esprimono eroi, mostri o edifici.

---

## ⚡️ Riassunto in 3 righe (TL;DR)

1. Usa `Low angle shot`, `Worm's eye view`.
2. Chiarisci l'elaborazione dello sguardo con la parola chiave `Looking up`.
3. Usare `Epic sky` come sfondo raddoppia la magnificenza.

---

## 🚀 La Soluzione: "Prompt Angolo Eroico"

### 🥉 Versione Base

Usala quando hai solo bisogno di un risultato rapido.

**Ruolo:** Sei un esperto.
**Richiesta:** Crea un prompt angolo basso che enfatizza dignità travolgente e dimensione guardando il soggetto dal basso.

<br>

### 🥇 Versione Pro (Esperto)

Usala quando hai bisogno di qualità dettagliata.

Copia il contenuto del **PROMPT** qui sotto e usalo.

> **Prompt:**
>
> **Subject:** A giant robot paladin standing in a destroyed city, holding a glowing shield.
>
> **Angle:** Extreme low angle shot, worm's eye view, looking up from the ground, imposing perspective.
>
> **Style:** Cinematic masterpiece, epic scale, lens flare, 8k, detailed textures, dramatic sky.
>
> **Parameters (Midjourney):** --ar 2:3 --v 6.0 --stylize 500

---

## 🧬 Anatomia del Prompt (Perché funziona?)

---

## 📊 Prova: Prima e Dopo

### ❌ Prima (Input)

`Giant robot in city.`

(Risultato: Solo una vista a livello degli occhi dove un robot è in piedi, l'enormità non si sente bene)

### ✅ Dopo (Risultato)

Prompt `Low angle` applicato

(Risultato: Scala travolgente dove i piedi del robot sono davanti ai miei occhi e la testa sembra toccare il cielo alto)

---

## 🎯 Conclusione

Se vuoi far guardare in alto gli altri, abbassa la fotocamera.

Esci dal lavoro in orario ora! 🍷
