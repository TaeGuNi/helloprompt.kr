...
- **🎯 Público objetivo:** Desarrolladores que reciben comentarios como "el color del botón no coincide con el diseño", o diseñadores frustrados pensando "el desarrollador ha arruinado mi diseño".
...
Los colores y márgenes definidos de forma estática (hardcoded) y dispersos por toda la base de código son los principales culpables de convertir el mantenimiento en un verdadero infierno. Es hora de implementar los **Design Tokens (Tokens de Diseño)**. Definir todos los elementos visuales —como colores, tipografías, márgenes y sombras— mediante variables semánticas compartidas es la garantía más sólida para alinear a tu equipo. Una vez que establezcas un ecosistema basado en tokens, experimentarás la magia de ver cómo el código del frontend se sincroniza automáticamente en el instante en que el diseñador modifica un valor en Figma.
...
2. **Aplicación de variables en Figma:** Aprovechamos las funciones de Variables y Styles de Figma para definir las propiedades visuales mediante tokens bien estructurados.
3. **Sincronización de código:** A partir de los datos extraídos, los convertimos automáticamente en el archivo `tailwind.config.ts` o en variables CSS globales para aplicarlos al proyecto frontend.
...
> Quiero dividir los colores `[Primario (Azul)]`, `[Secundario (Gris)]` y `[Error (Rojo)]` en 9 niveles cada uno, desde el 100 hasta el 900.
...
> **Contexto (Context):**
>
> - Antecedentes: Un diseñador ha definido los Design Tokens en Figma y los ha exportado en formato JSON.
...
> 2. Para no depender exclusivamente de un framework CSS, escribe también el código equivalente usando **Variables CSS nativas (`:root { --color-blue-500: ... }`)**.
> 3. Añade una estrategia y un código de ejemplo para soportar el modo oscuro (`@media (prefers-color-scheme: dark)` o mediante la clase `.dark`), asegurando que se adapte automáticamente al tema del sistema del usuario.
>
...
Construir un sistema de diseño no significa trazar un plan maestro gigantesco y terminarlo de una sola vez. Si intentas tokenizar a la perfección todos los componentes y la tipografía desde el primer día, es muy probable que te agotes, que el equipo pierda el impulso y que abandonen la iniciativa.
...
Ese pequeño primer token será el paso fundamental que reducirá drásticamente los costos de comunicación y las fricciones innecesarias entre diseñadores y desarrolladores.
...
- **P: ¿Qué plugin de Figma debo usar para extraer y exportar tokens?**
  - R: En la industria actual, **'Tokens Studio for Figma'** se utiliza como el estándar de facto. Permite extraer el JSON y sincronizarlo directamente con tu repositorio de GitHub mediante Pull Requests. Recientemente, la función nativa de Variables de Figma ha madurado muchísimo, por lo que la integración directa a través de su API REST también es una tendencia al alza.

- **P: Somos una startup muy pequeña de 2 o 3 personas, ¿realmente necesitamos invertir tiempo en un sistema de diseño?**
  - R: Si el equipo tiene 2 o más personas construyendo la interfaz de usuario, o si el mantenimiento del proyecto se extenderá por más de 3 meses, es absolutamente necesario. La deuda técnica de "lo ordenamos luego cuando tengamos tiempo" terminará convirtiéndose en una bomba de relojería que paralizará la velocidad del proyecto. Recuerda que tu "yo del futuro" también es, a efectos prácticos, otro desarrollador al que le estás dejando el problema.

---

## 🧬 Anatomía del Prompt (¿Por qué funciona?)

1. **Obligación de nomenclatura semántica:** Fomenta el uso de nombres basados en el contexto y el significado, como "color de fondo del botón principal (`bg-primary-default`)", en lugar de descripciones visuales literales como "azul claro". De esta manera, si la identidad de marca cambia de azul a morado en el futuro, solo necesitarás actualizar un valor Hex base para que se refleje instantáneamente en todo el producto.
...
No discutan más por píxeles sueltos o valores de color. Empiecen a comunicarse usando el idioma común e inequívoco de los tokens.
