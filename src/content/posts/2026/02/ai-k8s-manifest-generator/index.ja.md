...
> _「インデントが1つずれたせいでデプロイに失敗し、30分間もログと格闘した経験はありませんか？地獄のようなYAMLの記述は、もうすべてAIに完璧に外注しましょう。」_

Kubernetes（K8s）は現代のインフラ標準ですが、毎回ゼロからYAMLを記述するのは非常に苦痛を伴う作業です。`apiVersion`を間違えたり、`selector`と`labels`のマッチングでミスをしたり、複雑な公式ドキュメントを漁りながら既存のコードをコピペして致命的なヒューマンエラーを起こすことも少なくありません。

これからは、自然言語で「こういうスペックのサーバーを立ち上げて」と指示するだけにしましょう。AIがタイポなしで、ベストプラクティス（Best Practices）が適用された完璧なYAMLマニフェストをたった1秒で出力してくれます。
...
2. Deployment、Service、Ingressなど、複数のリソースを `---` 区切りで1つのファイルにまとめて作成できます。
...
> - **レプリカ数:** `[3]`
>
> **制約事項:** DeploymentとServiceを `---` で区切り、1つのファイルにまとめて出力してください。
...
> **制約事項 (Constraints):**
>
> - **Best Practicesの必須適用:** `livenessProbe` と `readinessProbe` を、適切なエンドポイント（例: `/actuator/health`）とデフォルトのタイムアウト値で必ず追加してください。
> - **Resource Management:** CPUとMemoryの `requests` および `limits` を、保守的なサンプル値で設定してください。
> - **Security Context:** コンテナが `root` 権限で実行されないように、 `runAsNonRoot: true` の設定を例として含めてください。
> - **出力形式:** 結果は必ずMarkdownのコードブロック（` ```yaml `）のみで出力してください。不要な前置きや挨拶は省略してください。
>
> **注意事項 (Warning):**
>
> - `apiVersion` は、最新のK8sバージョンに適合した安定版（stable）を使用してください（例: `apps/v1`、`networking.k8s.io/v1` など）。
