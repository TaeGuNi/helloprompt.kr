...
```markdown
### 🌪️ Guia Definitivo: Análise de Sensibilidade no Excel

**1. Preparando o Tabuleiro (Estrutura da Matriz)**

- Insira as variações da **Taxa de Câmbio** (5,00; 5,10; 5,20...) na Coluna A (verticalmente, a partir de A2).
- Insira as variações do **Volume de Vendas** (10k, 20k, 30k...) na Linha 1 (horizontalmente, a partir de B1).
- **A Chave de Ouro:** Na interseção exata (célula A1), você **deve** referenciar a fórmula do seu Lucro Operacional (ex: `=C10`).

**2. A Mágica Acontece (Executando a Tabela de Dados)**

- Selecione toda a matriz (de A1 até a última célula de dados).
- Navegue até: Aba **[Dados]** ➔ **[Teste de Hipóteses]** ➔ **[Tabela de Dados]**.
- **Célula de entrada da linha:** Selecione a célula original do _Volume de Vendas_ no seu modelo (pois seus cenários de volume estão espalhados na linha 1 horizontal).
- **Célula de entrada da coluna:** Selecione a célula original da _Taxa de Câmbio_ (pois seus cenários de câmbio estão descendo pela coluna A vertical).

**3. Interpretando os Resultados (Visão de CFO)**

- Ao clicar em OK, o Excel calculará instantaneamente todos os cruzamentos possíveis.
- **💡 Dica Estratégica:** Aplique Formatação Condicional > Escalas de Cor na matriz de resultados. Ao observar o mapa de calor, notamos que o lucro despenca para o negativo (zona vermelha) sempre que o câmbio ultrapassa os R$ 5,60, independentemente do volume de vendas. Isso indica uma urgência crítica para estabelecer contratos de _hedge_ cambial e proteger as margens da empresa.
```
