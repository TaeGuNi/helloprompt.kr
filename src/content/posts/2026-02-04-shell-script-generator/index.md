...
# (Updated) (Updated) 📝 서버 관리의 신, 쉘 스크립트(Shell Script) 자동 생성기 {#shell-script}
...
## (Updated) (Updated) 🚀 해결책: "Bash 마법사" {#updated}
...
### (Updated) (Updated) ❌ Before (입력 요구사항) {#updated}
...
```bash
#!/bin/bash
set -e  # (Updated) (Updated) 에러 발생 시 스크립트 중단

TARGET_DIR="images"
DATE_PREFIX=$(date +%Y%m%d)

# (Updated) (Updated) 1. 대상 디렉토리가 없으면 생성 {#updated}
if [ ! -d "$TARGET_DIR" ]; then
    echo "📁 '$TARGET_DIR' 디렉토리를 생성합니다..."
    mkdir -p "$TARGET_DIR"
fi

# (Updated) (Updated) 2. JPG 파일 이동 및 이름 변경 {#updated}
echo "🚀 파일 이동을 시작합니다..."
for file in *.jpg; do
    if [ -f "$file" ]; then
