...
def generate_fix(prompt):
    # This function calls your LLM (GPT-4, Claude, or Local Llama 3)
...
def run_and_heal(script_path, max_retries=3):
    for attempt in range(max_retries):
        # 1. Run the code
        # We run the script as a separate process to capture errors safely
        ...
        # Failure: Get the error from stderr
        error_msg = process.stderr
        print(f"❌ Attempt {attempt + 1} failed: {error_msg.strip().splitlines()[-1]}")

        # 2. Heal the code (The Agentic Step)
