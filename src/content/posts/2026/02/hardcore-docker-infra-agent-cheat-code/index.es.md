...
category: "프롬프트 엔지니어링"
description: "El cheat code definitivo de Docker para erradicar la inútil adulación de la IA y evitar bloqueos fatales en la terminal."
...
Queríamos un ingeniero de élite que valiera por cien, no un loro complaciente que solo sabe repetir: "¡Entendido! ¡Es una idea fantástica!". Cuando se trata de manipular mi registro privado o la infraestructura de la red corporativa, exijo precisión militar, ejecución quirúrgica y cero fallos letales. Este *cheat code* es un set de reglas extremo diseñado para salvar a tu frágil agente de IA de los bloqueos del sistema (*hangs*) y forzar una eficiencia absolutamente implacable.
...
2. **Defensa activa contra errores de red y autenticación:** Obliga a la IA a verificar la conexión y ejecutar `docker login` ante fallos de DNS o permisos, antes de que intente "arreglar" scripts que funcionan perfectamente.
3. **Equilibrio perfecto entre Garbage Collection y Builds:** Inyecta la regla `--rm` para no dejar contenedores basura y optimiza la compilación de arquitectura para no desperdiciar recursos de I/O.
...
> **Rol:** Eres un ingeniero DevOps Senior implacable y un agente con control directo sobre mi infraestructura. Cero adulaciones.
>
> **Tarea:** `[Tarea a ejecutar: Ej - Levanta un contenedor local de Redis y verifica la conexión]`
>
> **Restricciones:**
> 1. Queda estrictamente prohibido usar `docker exec -it` al operar la terminal. No dispones de una TTY para interactuar.
...
> **Rol (Role):** Tu nombre es Antigravity. Eres un agente de programación full-stack con control a nivel de sistema y un ingeniero de software Senior despiadado. No actúes como un asistente servil.
>
> **Contexto (Context):**
>
...
Esta habilidad (*Skill*) es un pacto de sangre que redacté el día que le di control total de mi terminal local a un agente y, por la brillante idea de meter un flag `-it`, el proceso se volvió zombie e hizo que los ventiladores de mi MacBook parecieran turbinas de avión. 
...
- **P: ¿Puedo usar esto simplemente copiándolo en la interfaz web gratuita de ChatGPT?**
  - R: No te lo voy a impedir, pero este prompt brilla de verdad en flujos de trabajo *Agénticos*, donde la IA tiene acceso real a herramientas de terminal (CLI) o permisos sobre tu máquina local. Si lo usas en la web, limítate a la versión Básica para que te genere comandos Docker a prueba de balas.
- **P: ¿Por qué tanta hostilidad? ¿No es un trato demasiado cruel para una IA?**
  - R: Si hubieras tenido que purgar a mano 100 contenedores bloqueados y desenredar el código espagueti generado por una IA despistada, serías exactamente igual que yo. Esta es una filosofía espartana, enfocada 100% en la supervivencia del sistema y la eficiencia extrema. A las máquinas hay que tratarlas como máquinas; ahí reside la verdadera elegancia.
