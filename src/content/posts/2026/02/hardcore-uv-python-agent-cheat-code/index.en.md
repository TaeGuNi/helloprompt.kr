...
category: "업무 자동화"
description: "A cheat code skill that blocks useless AI flattery and forces the ultra-fast Python environment (uv) for crushing, millisecond-level performance."
...
_"Are you still just watching your AI waste an eternity running `pip install`? Your agent is currently drowning in an I/O bottleneck."_

This post is a 'Spartan Cheat Code' designed to rebuild your AI agent into a snappy, powerhouse senior developer. I created this because I couldn't stand seeing them spout useless flattery like "Yes, I understand!", while wasting I/O on archaic relics like `pip` and `venv`. There is no mercy for the AI. We will force it to use only `uv`, the blazing-fast Rust-based package manager, and send your terminal response times into millisecond overdrive.

---

## ⚡️ TL;DR (3-Line Summary)
- Absolutely ban the use of outdated `pip` and `venv` (considered treason if caught).
- Force ultra-fast virtual environment setup and package installation exclusively through `uv`.
- Block useless nonsense (hallucinations) and secure overwhelming I/O performance.
...
2. Package Installation
   - ❌ PERMANENTLY BANNED: `pip install [package]` or `python -m pip install -r requirements.txt`
   - ✅ ABSOLUTELY RECOMMENDED: `uv pip install [package]` or `uv pip install -r requirements.txt`
...
4. One-off Script Execution
   ...
   - ✅ ABSOLUTELY RECOMMENDED: `uv run --with "[package]" script.py` (Runtime volatile execution)
...
>
> **Context:**
> - Background: A situation where the AI's response speed and environment setup have become terribly slow due to outdated `pip` and `venv`.
...
This prompt is an 'order' to rip out the agent's old habits. Once you inject this cheat code into the system, the AI will no longer spew useless flattery like "Yes, understood! I'm happy to help!". It awakens as a ruthless machine obsessed only with millisecond responsiveness. If you don't want your project ruined, confiscate its `pip` right now. We are here to code, not to watch terminal loading bars.
...
- ❓ **Q: I have an existing `requirements.txt`. Is it compatible?**
  - A: Of course. A single shot of `uv pip install -r requirements.txt` will slam them in 100 times faster than `pip`.
- ❓ **Q: The AI keeps stubbornly trying to use `pip`.**
  - A: That's because the system prompt weight is too low. Plant the keyword "**Treason**" in bold at the very top of the prompt to instill fear.
...
- ⚡️ **Specificity of Alternatives:** Rather than simply saying "use uv," it hands the agent perfect alternatives for every situation—dependency tracking (`uv add`), one-off execution (`uv run`)—eliminating any room for excuses.
...
### ❌ Before (Input)
The horrific sight when leaving a Python script execution to an agent:
...
## 🎯 Conclusion
AI pretends to be smart, but left to its own devices, it works in the laziest and stupidest ways possible. If you are a true senior, do not hand the choice of tools over to the AI.
