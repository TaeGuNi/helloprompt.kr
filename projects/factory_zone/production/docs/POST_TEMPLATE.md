---
layout: /src/layouts/Layout.astro
title: "여기에 제목을 입력하세요"
author: "Pumbaa"
date: "2026-02-14"
updatedDate: "2026-02-14"
category: "업무 자동화"
description: "검색엔진에 노출될 매력적인 한 줄 요약을 적어주세요."
tags: ["태그1", "태그2"]
image: "" # Unsplash Source URL (e.g., https://source.unsplash.com/1600x900/?tech,ai)
---

# 📝 여기에 메인 제목을 입력하세요

<!-- ⚠️ [Lint Rule] 이모지 리스트를 사용하세요. 표(Table) 사용 시 모바일에서 깨질 수 있습니다. -->

- **🎯 추천 대상:** 마케터, 기획자, 1년 차 신입사원
- **⏱️ 소요 시간:** 30분 → 1분 단축
- **🤖 추천 모델:** 모든 대화형 AI (ChatGPT, Claude, Gemini 등)

- ⭐ **난이도:** ⭐⭐☆☆☆
- ⚡️ **효과성:** ⭐⭐⭐⭐⭐
- 🚀 **활용도:** ⭐⭐⭐⭐☆

<!-- ⚠️ [Lint Rule] 인용구(>)는 Basic/Pro 섹션 외에는 이탤릭체(_..._)와 함께 사용해야 에러가 나지 않습니다. -->

> _"여기에 독자의 고통을 건드리는 훅(Hook) 한 문장을 적으세요."_

## 🌟 이 글을 읽어야 하는 이유 (Why This Matters)
- **시간 절약:** 이 프롬프트를 쓰면 퇴근 시간이 `[시간]` 빨라집니다.
- **퀄리티:** `[기존 방식]`보다 훨씬 더 정교한 결과를 얻을 수 있습니다.
- **당장 실행 가능:** 복사해서 붙여넣기만 하면 끝납니다.

---

## ⚡️ 3줄 요약 (TL;DR)

1. 첫 번째 핵심
2. 두 번째 핵심
3. 세 번째 핵심

---

## 🚀 해결책: "프롬프트 이름"

<!-- ⚠️ [Lint Rule] 인용구(>)는 이곳(Prompt 섹션)에서만 프롬프트 박스로 변환됩니다. -->

### 🥉 Basic Version (기본형)

빠르게 결과만 필요할 때 사용하세요.

> **역할:** 너는 `[전문가]`야.
> **요청:** `[문제]`를 해결해줘.

<br>

### 🥇 Pro Version (전문가형)

디테일한 퀄리티가 필요할 때 사용하세요.

> **역할 (Role):** 너는 `[전문가 역할]`이야.
>
> **상황 (Context):**
>
> - 배경: `[현재 상황]`
> - 목표: `[최종 목표]`
>
> **요청 (Task):**
>
> 1. 구체적인 지시사항 1
> 2. `[변수]` 부분은 사용자가 채워 넣을 수 있게 괄호로 표시하세요.
>
> **제약사항 (Constraints):**
>
> - 출력 형식은 마크다운 표(Table)로 해줘.
>
> **주의사항 (Warning):**
>
> - 확실하지 않은 정보는 지어내지 말고 "모른다"고 답해. (환각 방지)

---

<!-- ✅ [Lint Rule] 필수 섹션입니다. 누락 시 CI 에러가 발생합니다. -->

## 💡 작성자 코멘트 (Insight)

이 프롬프트가 왜 효과적인지, 실제 현업에서 어떻게 응용할 수 있는지 작성자의 통찰을 적어주세요.
(예: "이 방식은 특히 엑셀 데이터 정제할 때 유용합니다. 왜냐하면...")

---

<!-- ⚠️ [Lint Rule] 권장 섹션입니다. 누락 시 경고가 발생합니다. -->

## 🙋 자주 묻는 질문 (FAQ)

- **Q: 무료 버전 ChatGPT에서도 되나요?**
  - A: 네, 가능합니다. 다만 Pro 버전이 더 정교합니다.

- **Q: 다른 언어로도 되나요?**
  - A: 프롬프트 내의 언어 설정만 바꾸면 됩니다.

---

## 🧬 프롬프트 해부 (Why it works?)

1.  **Role 부여:** AI에게 페르소나를 씌워 전문성을 높였습니다.
2.  **Constraints(제약):** "하지 말아야 할 것"을 명시하여 환각(Hallucination)을 방지했습니다.

---

## 📊 증명: Before & After

### ❌ Before (입력)

```text
(엉망인 텍스트)
```

### ✅ After (결과)

```text
(완벽한 결과물)
```

---

## 📚 함께 보면 좋은 프롬프트 (Related Posts)

*   [관련 글 1 제목](/posts/related-post-1)
*   [관련 글 2 제목](/posts/related-post-2)
*   [관련 글 3 제목](/posts/related-post-3)

---

## 🎯 결론

마무리 멘트.

이제 칼퇴하세요! 🍷
