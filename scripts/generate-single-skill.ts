...
You MUST write this blog post in a fun, engaging, and slightly humorous tone that perfectly reflects this "mad scientist / spartan senior" persona! Make it entertaining and slightly cynical, but ultimately very helpful. Emphasize how this "cheat code" strips away AI's meaningless flatteries, and forces it to operate at an elite, merciless level.
...
async function main() {
  try {
    ...
    if (!skillName) {
      console.error("❌ Error: You must provide a single skill name.");
      process.exit(1);
    }

    console.log(`\n==========================================`);
    console.log(`🚀 Processing Single Skill: ${skillName}`);
    console.log(`==========================================\n`);
    ...
    await runCmd(
      `git commit -m "feat(content): generate skill post for ${skillName} with spartan persona"`,
    );
    await runCmd(`git push origin develop`);

    console.log(
      "🎉 All Done for this skill! Direct commit to develop applied.",
    );
  } catch (e) {
