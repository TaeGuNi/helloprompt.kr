...
- **⏱️ Tempo de execução:** 30 minutos → 3 minutos
- **🤖 Modelo recomendado:** Qualquer modelo de ponta (GPT-4o, Claude 3.5 Sonnet)
...
A disparidade entre o ambiente de desenvolvimento local e o servidor de produção é, sem dúvida, o maior pesadelo de qualquer equipe de engenharia. Todos sabemos que o Docker é a solução definitiva. No entanto, escrever um `Dockerfile` do zero, configurar redes e volumes no `docker-compose.yml` e ainda otimizar o tamanho da imagem exige um conhecimento profundo de DevOps.

Não perca horas vasculhando documentações extensas. Basta informar a stack de tecnologia do seu projeto, e a IA construirá uma infraestrutura Docker de nível de produção em questão de segundos.
...
2. **Orquestração Completa:** Escreve um `docker-compose.yml` pronto para uso, integrando perfeitamente a aplicação, o banco de dados e o cache.
3. **Otimização de Produção:** Aplica técnicas avançadas, como _Multi-stage Build_ e configurações rigorosas de segurança (usuário _non-root_).
...
Use esta versão quando precisar apenas de um esqueleto rápido para rodar testes locais.

> **Papel:** Você é um Engenheiro DevOps sênior especializado em conteinerização.
> **Solicitação:** Crie um `Dockerfile` simples e um `docker-compose.yml` para uma aplicação construída em `[Sua Stack de Tecnologia, ex: Node.js e MongoDB]`.

### 🥇 Versão Especialista (Pro)

Use esta versão para obter arquivos de configuração de nível de produção, com foco absoluto em segurança, performance e imagens ultraleves.
...
Na rotina de engenharia, a maior dor de cabeça ao iniciar a conteinerização não é simplesmente fazer a aplicação rodar, mas sim garantir que a imagem final não ultrapasse os 2GB e não fique vulnerável a ataques.

Este prompt _Pro_ é extremamente valioso porque força a IA a aplicar o padrão ouro da indústria: os **Multi-stage builds**. Ele compila o código em uma imagem temporária e transfere apenas os artefatos estritamente necessários para a imagem final, deixando-a incrivelmente leve. Além disso, a instrução inegociável de criar um usuário _non-root_ previne ataques de escalonamento de privilégios caso o contêiner seja comprometido. É literalmente como ter um Arquiteto Cloud revisando sua infraestrutura em poucos segundos!
...
- **Q: Preciso assinar alguma versão paga para esse prompt funcionar?**
...
1. **Role (Papel):** Ao definir a IA como "Engenheiro DevOps Sênior", elevamos drasticamente o nível técnico da resposta, forçando a adoção de boas práticas da indústria em vez de soluções voltadas para iniciantes.
...
3. **Prevenção de Vazamento de Contexto:** Solicitar a geração do `.dockerignore` é um detalhe crucial e frequentemente negligenciado. Isso evita que diretórios pesados como `node_modules` ou arquivos com variáveis de ambiente (`.env`) sejam copiados acidentalmente, o que comprometeria a segurança da imagem.
...
Ao empacotar sua aplicação e todas as dependências isoladamente em um contêiner, você garante uma execução idêntica em qualquer ambiente, seja no notebook de um novo membro da equipe ou em um cluster Kubernetes na nuvem. Use a IA para lidar com o trabalho pesado de infraestrutura e concentre-se no que realmente importa: escrever um código excelente! 🍷
