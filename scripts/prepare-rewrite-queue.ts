...
const TARGET_LANGS = ["de", "en", "es", "fr", "it", "ja", "ko", "pt", "ru", "zh"];
...
async function main() {
  ...
  for (const dir of dirs) {
    ...
    // Attempt mapping
    let existingLangs = existingFiles.map((f: string) => {
  ...
  console.log(`Prepared queue with ${missingFilesToRewrite.length} missing translation files.`);
}
