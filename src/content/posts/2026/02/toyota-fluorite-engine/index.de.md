...
```text
Wie mache ich meine Flutter-App schneller, wie diese Toyota-Engine?
```

### ✅ Nachher (Ergebnis mit dem Pro-Prompt)

```text
Hier ist eine Architekturanalyse zur Erreichung von High-Performance-Rendering ähnlich wie Fluorite:

- Rendering: Custom RenderObjects & Impeller anstelle der Standard Canvas API
- State: ValueNotifiers für lokalisierte Updates anstelle von setState() in großen Bäumen
- 3D Assets: Direkte FFI-Aufrufe an C++ / Rust anstelle von externen Plugins mit Bridge-Overhead

Um dein [interaktives 3D-Dashboard] zu optimieren, sollten wir von Standard-Widgets auf ein CustomRenderObject umsteigen. Hier ist der optimierte Code-Ansatz:
(Es folgt hochoptimierter Dart-Code)
```
