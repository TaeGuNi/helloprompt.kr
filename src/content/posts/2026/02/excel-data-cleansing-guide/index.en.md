...
Data cleaning is often the most time-consuming part of any analysis. Standardizing messy, inconsistent data formats is a mind-numbing chore that drains your productivity. Let's automate the grunt work so you can finally focus on the actual analysis.
...
2. Instantly apply this to your daily workflow, from Excel spreadsheets to weekly reports.
...
> 
> **Task:** Provide the best method to clean my dataset, specifically focusing on separating addresses and standardizing date formats.

### 🥇 Pro Version (Expert Mode)

Use this when you need a highly detailed, step-by-step tutorial complete with error handling.

> **Role:** You are a senior Data Analyst and Excel Master.
>
> **Context:**
>
> - Background: I have raw data mixed within Excel Column A (e.g., `[Insert Data Example]`).
...
> **Warning:**
>
> - If you are unsure about a specific function's behavior in the latest version of Excel, clearly state that you do not know. Do not hallucinate formulas.

---

## 💡 Writer's Insight

This prompt is an absolute lifesaver when dealing with messy CRM exports or survey results. The magic lies in asking for the **most efficient method**. Often, we default to complex, nested `FIND` and `MID` functions when a simple `Ctrl + E` (Flash Fill) can do the job in a fraction of a second. By constraining the AI to provide step-by-step instructions tailored for beginners, you aren't just getting a formula—you're getting a reusable mini-tutorial that genuinely upgrades your Excel skills. Always remember to ask for validation methods. Data is rarely perfectly uniform, and anticipating edge cases saves you from silent errors down the line.
...
- **Q: What if my data is extremely inconsistent (e.g., some cells have zip codes, some don't)?**
  - A: In that case, add those specific edge cases into the `[Insert Data Example]` section of the prompt. The AI will generate a more robust formula (like wrapping it in `IFERROR()`) to handle the discrepancies gracefully.
