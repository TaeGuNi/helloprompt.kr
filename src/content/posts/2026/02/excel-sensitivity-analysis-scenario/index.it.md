...
## 🚀 La Soluzione: "Generatore di Matrici di Scenari"
...
```text
Come faccio a vedere in excel cosa succede al profitto se cambiano le vendite e il tasso di cambio?
```

### ✅ Dopo (Risultato dal nostro Prompt Pro)

```text
### 🌪️ Guida Pratica all'Analisi di Sensibilità in Excel

**1. Struttura la tua Matrice (Il Setup Iniziale)**
- Inserisci gli scenari per il **Volume delle Vendite** (10k, 20k...) in Orizzontale, a partire dalla cella B1.
- Inserisci gli scenari per il **Tasso di Cambio** (1200, 1250...) in Verticale, a partire dalla cella A2.
- ⚠️ **Passaggio Cruciale:** Nella cella d'angolo A1 (dove riga e colonna si incontrano), inserisci un riferimento alla cella che calcola il tuo **Utile Operativo Finale** (es. `=D15`).

**2. Attiva la Tabella Dati (Il Calcolo Magico)**
- Seleziona l'intera matrice (da A1 fino all'ultima riga/colonna dei tuoi dati).
- Vai su: **Scheda [Dati] ➔ [Analisi di simulazione] ➔ [Tabella dati]**.
- **Cella di input per riga:** Seleziona la cella delle assunzioni originali relative al Volume delle Vendite (poiché l'hai posizionato in orizzontale).
- **Cella di input per colonna:** Seleziona la cella delle assunzioni originali relative al Tasso di Cambio (poiché l'hai posizionato in verticale).

**3. Insight Strategico (Controllo dei Risultati)**
- Nel momento in cui premi OK, le celle vuote si riempiranno magicamente con decine di stime di profitto.
- 💡 **Consiglio Pro:** Applica la *Formattazione Condizionale (Scale di Colori)*. La 'linea rossa' dove l'utile diventa negativo si vedrà a colpo d'occhio. Se il profitto crolla sopra un tasso di 1.350, è il momento di suggerire una strategia di copertura (Hedging).
```
