...
> **Contexto (Context):**
>
> - Cenário: Escrevi sobre a minha experiência em um projeto para o meu currículo, mas o texto está muito descritivo, passivo e as minhas reais realizações não ganham destaque.
> - Objetivo: Quero reescrever este texto aplicando a técnica STAR (Situação, Tarefa, Ação, Resultado) para criar um impacto imediato e gerar autoridade perante os recrutadores.
>
> **Tarefa (Task):**
>
> 1. Analise a minha experiência fornecida abaixo e reestruture-a utilizando o **framework STAR**.
> 2. Dê ênfase máxima aos números e métricas na seção **Result (Resultado)** (ex: % de crescimento, valor financeiro, tempo economizado). Se os números exatos não forem fornecidos ou estiverem vagos, sugira os tipos de indicadores realistas que eu deveria tentar estimar e inserir.
> 3. Por fim, recomende 3 **Tópicos de Impacto de Uma Única Frase (Bullet Points)** que eu possa copiar e colar diretamente no meu currículo.
>
> **Experiência de Entrada (Input):**
> [Insira a sua experiência em formato de texto livre aqui. Ex: Trabalhei em uma campanha de marketing no Instagram, ganhamos vários seguidores e o engajamento foi legal. Meu chefe gostou do resultado.]
>
...
### ❌ Antes (O seu rascunho inicial)

```text
Planejei e cuidei de um evento no Instagram na equipe de marketing.
Muitas pessoas participaram, ganhamos mais seguidores e fui elogiado pelo meu gerente.
```

### ✅ Depois (O resultado gerado pela IA)

```text
• **Alcancei um aumento de 150%** no número de seguidores em comparação ao mês anterior, planejando e executando uma campanha interativa no Instagram.
• **Otimizei** a taxa de conversão do evento em **25%** e gerei um efeito viral significativo ao simplificar o processo de participação do usuário.
• **Reduzi em 30% o CPC (Custo por Clique)** e melhorei o ROI geral ao segmentar anúncios baseados em dados reais de engajamento da campanha.
```
