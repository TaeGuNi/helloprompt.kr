...
## (Updated) (Updated) Unsloth란 무엇인가요?
...
max_seq_length = 2048
dtype = None # (Updated) (Updated) None으로 설정하면 자동으로 감지합니다.
load_in_4bit = True # (Updated) (Updated) 메모리 절약을 위해 4비트 양자화 사용
...
model = FastLanguageModel.get_peft_model(
    model,
    r = 16, # (Updated) (Updated) 8, 16, 32, 64, 128 중 선택
    target_modules = ["q_proj", "k_proj", "v_proj", "o_proj",
                      "gate_proj", "up_proj", "down_proj",],
    lora_alpha = 16,
    lora_dropout = 0, # (Updated) (Updated) 최적화를 위해 0 권장
    bias = "none",    # (Updated) (Updated) 최적화를 위해 "none" 권장
    use_gradient_checkpointing = "unsloth", # (Updated) (Updated) 긴 컨텍스트 처리에 필수
    random_state = 3407,
    use_rslora = False,
    loftq_config = None,
)
...
### (Updated) (Updated) Instruction:
{}

### (Updated) (Updated) Input:
{}

### (Updated) (Updated) Response:
{}"""

EOS_TOKEN = tokenizer.eos_token # (Updated) (Updated) 문장 종료 토큰 추가 필수!
...
trainer = SFTTrainer(
    ...
    args = TrainingArguments(
        ...
        warmup_steps = 5,
        max_steps = 60, # (Updated) (Updated) 테스트용으로 짧게 설정
        learning_rate = 2e-4,
...
[
    alpaca_prompt.format(
        "피보나치 수열을 파이썬으로 작성해줘.", # (Updated) (Updated) Instruction
        "", # (Updated) (Updated) Input
...
# (Updated) (Updated) 저장 (LoRA 어댑터만 저장) {#lora}
model.save_pretrained("lora_model")
# (Updated) (Updated) GGUF로 저장 (선택 사항)
# (Updated) (Updated) model.save_pretrained_gguf("model", tokenizer, quantization_method = "q4_k_m")
```
