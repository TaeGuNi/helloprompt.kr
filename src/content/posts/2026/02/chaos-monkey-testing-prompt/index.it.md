...
Se chiedi a un agente IA di scrivere dei test, nove volte su dieci ti restituirà una serie di rassicuranti "scenari di funzionamento ideale", i classici Happy Path. Un mondo utopico in cui le API esterne rispondono in 0,1 secondi, il database non si disconnette mai e gli utenti inseriscono sempre dati immacolati. È ora di darci un taglio. Il codice che dobbiamo rilasciare è uno scudo termico, progettato per resistere a quell'80% di fallimenti e disastri imprevedibili che si verificano nella realtà. Il cheat code che ti presento in questo articolo è un prompt estremo: prende l'IA per il colletto e le inietta a forza la spietata filosofia "Chaos Monkey" di Netflix.

---

## ⚡️ Sintesi in 3 righe (TL;DR)

- 🚫 **L'Happy Path vale solo il 20%:** I test che passano sempre sono solo una pericolosa facciata; dedica l'80% del tuo sforzo a difendere il sistema dagli scenari di fallimento.
- 💥 **Simulazione di disastri forzata:** Esigi l'inclusione di timeout di rete, errori HTTP 500 e disconnessioni del database all'interno dei blocchi di test.
- 🧹 **Niente mocking sporco (Dirty Mocking):** Punisci severamente chi inquina oggetti globali come `window.fetch` e pretendi un isolamento chirurgico tramite MSW o Dependency Injection.
...
Usalo quando hai bisogno di aggiungere rapidamente una suite di test incentrata esclusivamente sulla gestione degli errori.

> **Ruolo:** Sei un QA Engineer hardcore, spietato come un Chaos Monkey.
> **Richiesta:** Scrivi il codice di test per il `[codice da testare]` che ti ho fornito, includendo un solo scenario di funzionamento normale (Happy Path). Tutti gli altri test devono simulare "gli scenari di disastro peggiori" (es. errori di rete 500, timeout del DB o input malformati) per verificare la robustezza della logica di difesa.

### 🥇 Pro Version (Versione Esperto)

Questo è il cheat code definitivo. Spazza via il servilismo senz'anima dell'IA, forzandola a generare test isolati a livello di produzione, completi di verifica della garbage collection. Copialo e incollalo così com'è.
...
>
> **Richiesta (Task):**
> 1. Limita gli scenari Happy Path a meno del 20% dei test totali. Il tuo vero valore risiede nella validazione del restante 80%, composto unicamente da scenari di fallimento.
...
Siamo onesti: quando chiedi a un'IA di scrivere del codice di test, farà di tutto per rifugiarsi nella sua zona di comfort pur di mostrarti quell'illusorio "segno di spunta verde (✅)". Un'utopia in cui le API rispondono in 1 millisecondo e gli utenti inviano solo payload JSON immacolati. 
...
- **Q: Non posso usare `jest.mock()` al posto di MSW?**
  - A: Mockare i moduli va benissimo. Ciò che è intollerabile è la cattiva abitudine di alterare arbitrariamente l'oggetto `global`, infettando come un virus anche gli altri file di test. Finché garantisci il perfetto isolamento (Isolation) dello scope, puoi usare lo strumento che preferisci.
...
- **Risultato:**
  ...
  - Conferma che, in caso di crash del database con errore 500, venga renderizzata una Fallback UI sicura per l'utente.
